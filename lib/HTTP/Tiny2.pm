...
=method new
    ...
    $http = HTTP::Tiny2->new( %attributes );

This constructor returns a new HTTP::Tiny2 object.  Valid attributes include:
...
across multiple destinations, use multiple HTTP::Tiny2 objects.
...
=method can_ssl
    ...
    $ok         = HTTP::Tiny2->can_ssl;
    ($ok, $why) = HTTP::Tiny2->can_ssl;
    ($ok, $why) = $http->can_ssl;
...
sub can_ssl {
    ...
    if ( ref($self) && ( $self->{verify_SSL} || $self->{SSL_options}{SSL_verify_mode} ) ) {
        my $handle = HTTP::Tiny2::Handle->new(
...
package
    HTTP::Tiny2::Handle; # hide from PAUSE/indexers
use strict;
use warnings;
...
sub _assert_ssl {
    my($ok, $reason) = HTTP::Tiny2->can_ssl();
...
=head1 SYNOPSIS
    ...
    my $response = HTTP::Tiny2->new->get('http://example.com/');
...
If L<IO::Socket::IP> 0.25 or later is installed, HTTP::Tiny2 will use it instead
...
encryption fails. You can also use C<HTTP::Tiny2::can_ssl()> utility function
...
B<By default, HTTP::Tiny2 does not verify server identity>.
...
By default, HTTP::Tiny2 does not make any assumptions about your trust model,
threat level or risk tolerance.  It just aims to give you an encrypted channel
when you need one.

Setting the C<verify_SSL> attribute to a true value will make HTTP::Tiny2 verify
...
If the environment variable C<SSL_CERT_FILE> is present, HTTP::Tiny2
will try to find a CA certificate file in that location.

If the L<Mozilla::CA> module is installed, HTTP::Tiny2 will use the CA file
included with it as a source of trusted CA's.  (This means you trust Mozilla,
the author of Mozilla::CA, the CPAN mirror where you got Mozilla::CA, the
toolchain used to install it, and your operating system security, right?)

If that module is not available, then HTTP::Tiny2 will search several
...
C<IO::Socket::SSL::start_SSL()>, overriding any options set by HTTP::Tiny2. For
...
HTTP::Tiny2 can proxy both C<http> and C<https> requests.  Only Basic proxy
authorization is supported and it must be provided as part of the proxy URL:
C<http://user:pass@proxy.example.com/>.

HTTP::Tiny2 supports the following proxy environment variables:
...
HTTP::Tiny2 is I<conditionally compliant> with the
...
HTTP::Tiny2 focuses on correct transport.  Users are responsible for ensuring
...
Despite the limitations listed above, HTTP::Tiny2 is considered
...
=for :list
* L<HTTP::Tiny2::UA> - Higher level UA features for HTTP::Tiny2
* L<HTTP::Thin> - HTTP::Tiny2 wrapper with L<HTTP::Request>/L<HTTP::Response> compatibility
* L<HTTP::Tiny2::Mech> - Wrap L<WWW::Mechanize> instance in HTTP::Tiny2 compatible interface
* L<IO::Socket::IP> - Required for IPv6 support
* L<IO::Socket::SSL> - Required for SSL support
* L<LWP::UserAgent> - If HTTP::Tiny2 isn't enough for you, this is the "standard" way to do things
